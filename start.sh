...
# 1. Vérification de la configuration initiale
if [ ! -d "data/models" ] || [ ! -f "data/models/embed.gguf" ] || [ ! -f ".env" ]; then
    echo "► Configuration initiale détectée comme incomplète..."
    bash "./setup.sh" --download-embed
fi

# Créer les répertoires de données restants
echo ""
echo "► Création des répertoires de données..."
mkdir -p data/documents data/dataset data/fine-tuning data/fine-tuning/merged data/models data/source
echo "  ✓ Structure de répertoires validée"
...
if [[ -n "$DETACH" ]]; then
    ...
    echo ""
    echo " Ajouter des documents : ./adddoc.sh"
    echo " Arrêt                 : ./stop.sh"
